const connection = require('../app/database');

class categoryService {
  async create(category) {
    console.log(category, 'category数据');
    // 1.获取category的数据
    const { name, commodityDesc, picture, brand, createT } = category;

    try {
      // 1. 参数校验
      if (!name) {
        throw new Error('Missing required fields: name, or picture');
      }

      // 2. 动态生成 SQL 语句
      const fields = [];
      const placeholders = [];
      const params = [];

      if (name !== undefined) {
        fields.push('name');
        placeholders.push('?');
        params.push(name);
      }
      if (commodityDesc !== undefined) {
        fields.push('commodityDesc');
        placeholders.push('?');
        params.push(commodityDesc);
      }
      if (brand !== undefined) {
        fields.push('brand');
        placeholders.push('?');
        params.push(brand);
      }
      if (picture !== undefined) {
        fields.push('picture');
        placeholders.push('?');
        params.push(picture);
      }
      if (createT !== undefined) {
        fields.push('createT');
        placeholders.push('?');
        params.push(createT);
      }

      // 如果没有有效的字段，抛出错误
      if (fields.length === 0) {
        throw new Error('No valid fields to insert');
      }

      // 3. 拼接 SQL 语句
      const statement = `INSERT INTO \`category\` (${fields.join(', ')}) VALUES (${placeholders.join(', ')})`;

      // 4. 调试：打印 SQL 语句和参数
      console.log('SQL:', statement);
      console.log('Params:', params);

      // 5. 执行 SQL 语句
      const [result] = await connection.execute(statement, params);

      // 6. 返回插入结果
      return {
        id: result.insertId, // 返回插入的 ID
        ...(name && { name }),
        ...(commodityDesc && { commodityDesc }),
        ...(brand && { brand }),
        ...(picture && { picture }),
        ...(createT && { createT })
      };
    } catch (error) {
      console.error('SQL Error:', error);
      throw new Error('Failed to add category info');
    }
  }

  async list({ name, page = 1, pageSize = 10 }) {
    console.log(name, page, pageSize, 'data数据');

    try {
      // 查询总条数
      let countSql = 'SELECT COUNT(*) AS total FROM category';
      const countParams = [];

      // 查询当前页数据
      let sql = 'SELECT * FROM category';
      const params = [];

      // 动态拼接 WHERE 子句
      const conditions = [];
      if (name) {
        conditions.push('name LIKE ?');
        params.push(`%${name}%`);
        countParams.push(`%${name}%`);
      }

      // 如果有搜索条件，拼接 WHERE 子句
      if (conditions.length > 0) {
        const whereClause = ' WHERE ' + conditions.join(' AND ');
        sql += whereClause;
        countSql += whereClause;
      }

      // 分页逻辑
      const offset = (page - 1) * pageSize;

      // 使用直接插入的方式来设置 LIMIT 和 OFFSET
      sql += ` LIMIT ${Number(pageSize)} OFFSET ${Number(offset)}`;

      // 调试：打印 SQL 语句和参数
      // console.log('Count SQL:', countSql);
      // console.log('Count Params:', countParams);
      // console.log('Data SQL:', sql);
      // console.log('Data Params:', params);

      // 执行查询
      const [countResult] = await connection.execute(countSql, countParams);
      const total = countResult[0].total;
      const [rows] = await connection.execute(sql, params);

      // 返回分页信息
      return {
        total,
        page,
        pageSize,
        list: rows
      };
    } catch (error) {
      console.error('SQL Error:', error);
      throw new Error('Failed to fetch category list');
    }
  }

  async update(data) {
    const { id, name, commodityDesc, brand, picture, updateT } = data;

    try {
      // 初始化 SQL 语句和参数
      let sql = 'UPDATE `category` SET ';
      const params = [];
      const updates = [];

      // 动态添加需要更新的字段
      if (name !== undefined) {
        updates.push('name = ?');
        params.push(name);
      }
      if (commodityDesc !== undefined) {
        updates.push('commodityDesc = ?');
        params.push(commodityDesc);
      }
      if (brand !== undefined) {
        updates.push('brand = ?');
        params.push(brand);
      }
      if (picture !== undefined) {
        updates.push('picture = ?');
        params.push(picture);
      }
      if (updateT !== undefined) {
        updates.push('updateT = ?');
        params.push(updateT);
      }

      // 如果没有需要更新的字段，直接返回
      if (updates.length === 0) {
        throw new Error('No fields to update');
      }

      // 拼接 SQL 语句
      sql += updates.join(', ') + ' WHERE id = ?';
      params.push(id);

      // 调试：打印 SQL 语句和参数
      console.log('SQL:', sql);
      console.log('Params:', params);

      // 执行 SQL 语句
      const [result] = await connection.execute(sql, params);
      return result;
    } catch (error) {
      console.error('SQL Error:', error);
      throw new Error('Failed to update category');
    }
  }

  async remove(id) {
    const statement = `DELETE FROM category WHERE id = ?`;
    const [result] = await connection.execute(statement, [id]);
    return result;
  }

  async queryBrand() {
    const statement = `SELECT id, brand, name FROM category;`;
    const [result] = await connection.execute(statement);
    return result;
  }
}

module.exports = new categoryService();
